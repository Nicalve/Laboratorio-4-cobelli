...
ROOT = Path(__file__).resolve().parents[1] #asi funciona para spyder
...
for rhos in todos_los_rhos:
    todos.extend(rhos)

rho_prom = np.mean(todos)

print("\nRho promedio:", rho_prom)
...
for i, rhos in enumerate(todos_los_rhos):
    ...
    mejor = min(rhos, key=lambda x: abs(x - rho_prom))

    L = datos_segun_L[i][0]

    print(f"L = {L:.3f} -> rho = {mejor:.3e}")
...
L=[(0.85*2)-0.121,(0.85*2)-0.187,(0.85*2)-0.261,(0.85*2)-0.27,(0.85*2)-0.36,(0.85*2)-0.969,(0.85*2)-1.060,(0.85*2)-1.35]
